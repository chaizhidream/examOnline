		...
		<script type="text/javascript">
			...
			$(function() {
				...
				$('#grid').datagrid({
					...
					pagination : true,
					toolbar: toolbar,
					url: "/manager-web/adminExam/showFormalExam",
					idField: 'examId',
					columns: columns,
			...
			function doView() {
				...
				console.info(examList.examGrade);
				window.location.href = "/manager-web/page/menupage/formalexamContent.jsp?examid=" + examList.examId +"&examName=" +examList.examName+"&examGrade=" +examList.examGrade+"&state=" +examList.state;
			}
			...
			function do666() {
				...
				$.ajax({
						url: "/manager-web/adminExam/exitformalExamId?examid=" + examList.examId,
						type: "post",
						dataType: 'JSON',
						success: function(result) {
							...
							if(result == "666") {
								...
								$.messager.alert("警告", "设置失败!", "warning");
							}
						}
					});
			...
			function doDelete() {
				...
				if(items.length == 0) {
					...
					for(var i = 0; i < items.length; i++) {
						...
						examGrade = items[i].examGrade;
					}
					console.log(ids)
					window.location.href = "/manager-web/adminExam/deleteExam?ids=" + ids +"&examGrade=" + examGrade;
				}
